    ...
    let health_handle = if config.health_port > 0 {
        ...
        .await
        {
            Ok(server) => {
                ...
                // Wire the readiness probe to query the Raft coordinator
                // directly, bypassing the up-to-1s lag of the global
                // `RAFT_STATE` gauge.
                let server = server.with_raft_state_provider(handler.raft_coordinator());
                info!(
                    broker_id = config.broker_id,
