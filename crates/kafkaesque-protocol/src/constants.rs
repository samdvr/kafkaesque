...
/// Offset of the batch_length field (4 bytes, i32 big-endian).
/// `batch_length` counts bytes from byte 12 (partition_leader_epoch) to the
/// end of the batch — the total batch size on the wire is `12 + batch_length`.
pub const BATCH_LENGTH_OFFSET: usize = 8;

/// End offset of the batch_length field (exclusive).
pub const BATCH_LENGTH_END: usize = 12;

/// Number of header bytes preceding the region counted by `batch_length`.
/// Total batch size = `BATCH_LENGTH_PREFIX + batch_length`.
pub const BATCH_LENGTH_PREFIX: usize = 12;
