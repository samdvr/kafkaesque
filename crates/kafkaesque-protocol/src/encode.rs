...
use crate::constants::{
    DEFAULT_MAX_MESSAGE_SIZE, MAX_PROTOCOL_ARRAY_SIZE, MAX_PROTOCOL_STRING_SIZE,
};
...
fn encode_bytes_len<T: BufMut>(buffer: &mut T, len: usize) -> Result<()> {
    // Match the parser's per-buffer cap so a response we successfully encode
    // round-trips through the same library. The wire field is i32, but the
    // parser rejects byte arrays larger than MAX_PROTOCOL_STRING_SIZE.
    if len > MAX_PROTOCOL_STRING_SIZE as usize {
        return Err(Error::Config(format!(
            "byte length {} exceeds protocol maximum {}",
            len, MAX_PROTOCOL_STRING_SIZE
        )));
    }
...
fn encode_array_len<T: BufMut>(buffer: &mut T, len: usize) -> Result<()> {
    // Match the parser's element-count cap so a response we successfully
    // encode round-trips through the same library. Wire field is i32, but
    // parse_array rejects anything above MAX_PROTOCOL_ARRAY_SIZE.
    if len > MAX_PROTOCOL_ARRAY_SIZE as usize {
        return Err(Error::Config(format!(
            "array length {} exceeds protocol maximum {}",
            len, MAX_PROTOCOL_ARRAY_SIZE
        )));
    }
    buffer.put_i32(len as i32);
    Ok(())
...
/// Public version of `encode_array_len` for callers that build their array
/// elements via a different mechanism than the `ToByte` trait — for example
/// the chain-based fetch encoder, which writes elements into a mix of an
/// accumulating `BytesMut` and a `BytesChain` of zero-copy slices.
#[inline]
pub fn encode_array_len_pub<T: BufMut>(buffer: &mut T, len: usize) -> Result<()> {
    encode_array_len(buffer, len)
}
...
/// Encode a record-batch payload as Kafka BYTES.
///
/// Distinct from `Bytes::encode`: that path enforces
/// `MAX_PROTOCOL_STRING_SIZE` (16 MiB), which is appropriate for
/// string-shaped fields (topic / group / principal / sasl auth bytes)
/// but is far below the broker's configurable record-batch ceiling.
/// Record-batch payloads are bounded by `DEFAULT_MAX_MESSAGE_SIZE`
/// (100 MiB), matching Kafka's `message.max.bytes` default.
pub fn encode_records<W: BufMut>(buffer: &mut W, payload: &[u8]) -> Result<()> {
    if payload.len() > DEFAULT_MAX_MESSAGE_SIZE {
        return Err(Error::Config(format!(
            "record-batch payload {} exceeds protocol maximum {}",
            payload.len(),
    ...
    if payload.len() > i32::MAX as usize {
        return Err(Error::Config(format!(
            "record-batch payload {} exceeds wire field maximum {}",
            payload.len(),
            i32::MAX
        )));
    }
...
/// Append a record-batch payload to a [`crate::bytes_chain::BytesChain`]
/// without copying. The caller passes the already-refcounted payload
/// (typically straight from SlateDB) and we add two chunks to the chain:
/// a 4-byte length prefix and the payload's `Bytes` itself. Use this on
/// the fetch hot path to avoid the memcpy that `encode_records` performs
/// when building one contiguous buffer.
pub fn encode_records_into_chain(
    chain: &mut crate::bytes_chain::BytesChain,
    payload: bytes::Bytes,
) -> Result<()> {
    if payload.len() > DEFAULT_MAX_MESSAGE_SIZE {
        return Err(Error::Config(format!(
    ...
    let mut prefix = bytes::BytesMut::with_capacity(4);
    prefix.put_i32(payload.len() as i32);
    chain.push(prefix.freeze());
    chain.push(payload);
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_encode_array_len_rejects_above_protocol_cap() {
        let mut buf = Vec::new();
        let res = encode_array_len(&mut buf, MAX_PROTOCOL_ARRAY_SIZE as usize + 1);
        match res {
        ...
        encode_array_len(&mut buf, MAX_PROTOCOL_ARRAY_SIZE as usize)
            .expect("at limit must succeed");
    }

    #[test]
    fn test_encode_bytes_len_rejects_above_protocol_cap() {
        let mut buf = Vec::new();
        let res = encode_bytes_len(&mut buf, MAX_PROTOCOL_STRING_SIZE as usize + 1);
        match res {
            Err(Error::Config(msg)) => {
                assert!(msg.contains("exceeds protocol maximum"));
            }
