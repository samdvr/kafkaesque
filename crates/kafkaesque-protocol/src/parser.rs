    ...
    E: nom::error::ParseError<NomBytes>,
{
    move |input: NomBytes| {
        ...
        let cap = bounded_capacity(length, rest.as_bytes().len(), 1);
...
/// Compute a `Vec::with_capacity` hint that cannot exceed what the remaining
/// buffer could possibly populate. Without this bound a 24-byte hostile frame
/// can ask for a 100k-element pre-allocation.
fn bounded_capacity(length: i32, remaining: usize, min_element_size: usize) -> usize {
    let length = length.max(0) as usize;
    let remaining_bound = remaining / min_element_size.max(1);
    length.min(remaining_bound)
}
...
pub fn parse_unsigned_varint(s: NomBytes) -> IResult<NomBytes, u32> {
    ...
    loop {
        let b = match bytes.get(consumed) {
            Some(&b) => b,
            None => {
