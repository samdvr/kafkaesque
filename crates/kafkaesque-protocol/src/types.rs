...
impl Default for Offset {
    /// Returns the `INVALID` sentinel rather than `0`. A `0` default would
    /// silently alias the first valid offset with "unset", letting a
    /// `Default::default()`-initialized field point at a real record.
    fn default() -> Self {
        Self::INVALID
    }
}
...
impl Default for BrokerId {
    /// Returns the `INVALID` sentinel rather than `0`. A `0` default would
    /// point at broker 0 — a real broker in any single-broker cluster —
    /// instead of signaling `LeaderNotAvailable`.
    fn default() -> Self {
        Self::INVALID
    }
}
...
impl Default for PartitionIndex {
    /// Returns the `INVALID` sentinel rather than `0`. A `0` default would
    /// silently address partition 0 of any topic instead of signaling
...
impl GenerationId {
    ...
    ///
    /// Saturates at `i32::MAX` rather than wrapping. A wrap into negative
    /// space would eventually land on `-1`, which is the `INVALID`
    /// sentinel — silently aliasing a real generation with "no
    /// generation" and breaking every consumer-side equality check that
    /// uses `GenerationId::INVALID` to mean "unset".
    #[inline]
    pub fn next(self) -> Self {
        GenerationId(self.0.saturating_add(1))
    }
}

impl Default for GenerationId {
    /// Returns the `INVALID` sentinel rather than `0`. Generation `0` is a
    /// valid live generation, so a `0` default would silently match a
    /// real group state instead of signaling "no generation".
    fn default() -> Self {
        Self::INVALID
    }
}
...
impl Default for ProducerId {
    /// Returns the `INVALID` sentinel rather than `0`. The first allocated
    /// producer id is `0`, so a `0` default would silently alias a real
    /// idempotent producer with "unset".
    fn default() -> Self {
        Self::INVALID
    }
}
...
impl Default for ProducerEpoch {
    /// Returns the `INVALID` sentinel rather than `0`. Epoch `0` is the
    /// first valid epoch a producer holds after initialization, so a `0`
    /// default would silently match a real producer's first incarnation.
    fn default() -> Self {
        Self::INVALID
    }
}
...
///
/// The topic is stored as `Arc<str>` so cloning a `PartitionId` is a
/// pointer bump rather than a string copy — important on the hot path
/// where the same topic appears in many partitions.
///
/// # Usage
///
/// ```
...
impl From<PartitionId> for (String, i32) {
    fn from(id: PartitionId) -> Self {
        (id.topic.to_string(), id.partition)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_generation_id_next_saturates() {
        let id = GenerationId::new(i32::MAX);
        assert_eq!(id.next().value(), i32::MAX);
    }
    ...
    #[test]
    fn test_defaults() {
        assert_eq!(Offset::default().value(), -1);
        assert_eq!(BrokerId::default().value(), -1);
        assert_eq!(CorrelationId::default().value(), 0);
        assert_eq!(PartitionIndex::default().value(), -1);
        assert_eq!(GenerationId::default().value(), -1);
        assert_eq!(ProducerId::default().value(), -1);
        assert_eq!(ProducerEpoch::default().value(), -1);
    }
