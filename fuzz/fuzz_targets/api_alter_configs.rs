...
use kafkaesque::server::request::parse_alter_configs_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::AlterConfigs).expect("AlterConfigs is advertised");
    ...
    let _ = parse_alter_configs_request(NomBytes::new(body), version);
});
