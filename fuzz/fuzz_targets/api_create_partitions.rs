...
use kafkaesque::server::request::parse_create_partitions_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::CreatePartitions).expect("CreatePartitions is advertised");
    ...
    let _ = parse_create_partitions_request(NomBytes::new(body), version);
});
