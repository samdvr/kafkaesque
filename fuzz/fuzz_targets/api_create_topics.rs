...
use kafkaesque::server::request::parse_create_topics_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::CreateTopics).expect("CreateTopics is advertised");
    ...
    let _ = parse_create_topics_request(NomBytes::new(body), version);
});
