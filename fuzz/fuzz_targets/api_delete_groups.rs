...
use kafkaesque::server::request::parse_delete_groups_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::DeleteGroups).expect("DeleteGroups is advertised");
    ...
    let _ = parse_delete_groups_request(NomBytes::new(body), version);
});
