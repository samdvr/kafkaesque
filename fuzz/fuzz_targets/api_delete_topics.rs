...
use kafkaesque::server::request::parse_delete_topics_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::DeleteTopics).expect("DeleteTopics is advertised");
    ...
    let _ = parse_delete_topics_request(NomBytes::new(body), version);
});
