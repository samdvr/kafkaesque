...
use kafkaesque::server::request::parse_describe_groups_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::DescribeGroups).expect("DescribeGroups is advertised");
    ...
    let _ = parse_describe_groups_request(NomBytes::new(body), version);
});
