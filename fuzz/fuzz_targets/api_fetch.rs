...
use kafkaesque::server::request::parse_fetch_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::Fetch).expect("Fetch is advertised");
    ...
    let _ = parse_fetch_request(NomBytes::new(body), version);
});
