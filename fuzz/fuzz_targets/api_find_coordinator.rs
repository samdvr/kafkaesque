...
use kafkaesque::server::request::parse_find_coordinator_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::FindCoordinator).expect("FindCoordinator is advertised");
    ...
    let _ = parse_find_coordinator_request(NomBytes::new(body), version);
});
