...
use kafkaesque::server::request::parse_heartbeat_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::Heartbeat).expect("Heartbeat is advertised");
    ...
    let _ = parse_heartbeat_request(NomBytes::new(body), version);
});
