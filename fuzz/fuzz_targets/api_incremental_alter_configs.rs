...
use kafkaesque::server::request::parse_incremental_alter_configs_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::IncrementalAlterConfigs)
        .expect("IncrementalAlterConfigs is advertised");
    ...
    let _ = parse_incremental_alter_configs_request(NomBytes::new(body), version);
});
