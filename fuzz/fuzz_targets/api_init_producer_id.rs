...
use kafkaesque::server::request::parse_init_producer_id_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::InitProducerId).expect("InitProducerId is advertised");
    ...
    let _ = parse_init_producer_id_request(NomBytes::new(body), version);
});
