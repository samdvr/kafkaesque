...
use kafkaesque::server::request::parse_join_group_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::JoinGroup).expect("JoinGroup is advertised");
    ...
    let _ = parse_join_group_request(NomBytes::new(body), version);
});
