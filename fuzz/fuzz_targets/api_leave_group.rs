...
use kafkaesque::server::request::parse_leave_group_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::LeaveGroup).expect("LeaveGroup is advertised");
    ...
    let _ = parse_leave_group_request(NomBytes::new(body), version);
});
