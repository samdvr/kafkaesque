...
use kafkaesque::server::request::parse_list_groups_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::ListGroups).expect("ListGroups is advertised");
    ...
    let _ = parse_list_groups_request(NomBytes::new(body), version);
});
