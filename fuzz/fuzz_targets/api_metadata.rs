...
use kafkaesque::server::request::parse_metadata_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::Metadata).expect("Metadata is advertised");
    ...
    let _ = parse_metadata_request(NomBytes::new(body), version);
});
