...
use kafkaesque::server::request::parse_offset_commit_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::OffsetCommit).expect("OffsetCommit is advertised");
    ...
    let _ = parse_offset_commit_request(NomBytes::new(body), version);
});
