...
use kafkaesque::server::request::parse_offset_fetch_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::OffsetFetch).expect("OffsetFetch is advertised");
    ...
    let _ = parse_offset_fetch_request(NomBytes::new(body), version);
});
