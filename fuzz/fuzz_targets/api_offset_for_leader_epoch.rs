...
use kafkaesque::server::request::parse_offset_for_leader_epoch_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::OffsetForLeaderEpoch).expect("OffsetForLeaderEpoch is advertised");
    ...
    let _ = parse_offset_for_leader_epoch_request(NomBytes::new(body), version);
});
