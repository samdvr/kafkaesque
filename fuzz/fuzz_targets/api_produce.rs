...
use kafkaesque::server::request::parse_produce_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::Produce).expect("Produce is advertised");
    let span = (v.max_version - v.min_version + 1).max(1) as i16;
    // Use first 2 bytes as version selector across [min, max]; rest is body.
    ...
    let _ = parse_produce_request(NomBytes::new(body), version);
});
