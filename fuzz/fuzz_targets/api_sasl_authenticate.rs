...
use kafkaesque::server::request::parse_sasl_authenticate_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::SaslAuthenticate).expect("SaslAuthenticate is advertised");
    ...
    let _ = parse_sasl_authenticate_request(NomBytes::new(body), version);
});
