...
use kafkaesque::server::request::parse_sasl_handshake_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::SaslHandshake).expect("SaslHandshake is advertised");
    ...
    let _ = parse_sasl_handshake_request(NomBytes::new(body), version);
});
