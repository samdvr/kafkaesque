...
use kafkaesque::server::request::parse_sync_group_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::SyncGroup).expect("SyncGroup is advertised");
    ...
    let _ = parse_sync_group_request(NomBytes::new(body), version);
});
