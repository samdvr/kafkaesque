...
use kafkaesque::server::request::parse_api_versions_request;
...
fuzz_target!(|data: &[u8]| {
    ...
    let v = find_version(ApiKey::ApiVersions).expect("ApiVersions is advertised");
    ...
    let _ = parse_api_versions_request(NomBytes::new(body), version);
});
