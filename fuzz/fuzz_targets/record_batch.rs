...
use kafkaesque::protocol::{
    crc32c, parse_producer_info, parse_record_count, parse_record_count_checked,
    validate_batch_crc, CrcValidationResult,
};
...
fuzz_target!(|data: &[u8]| {
    ...
    if data.len() >= BATCH_CRC_DATA_START + 1 {
        ...
        if buf.len() > BATCH_CRC_DATA_START {
            ...
            match validate_batch_crc(&buf) {
                ...
                CrcValidationResult::Valid => panic!(
                    "bit-flip in CRC-covered region was not detected by validate_batch_crc"
                ),
    ...
    if data.len() >= MIN_HEADER {
        if let Some(info) = parse_producer_info(data) {
