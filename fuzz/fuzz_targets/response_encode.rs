...
fn arb_metadata_response(u: &mut Unstructured<'_>) -> arbitrary::Result<MetadataResponseData> {
...
fn arb_produce_response(u: &mut Unstructured<'_>) -> arbitrary::Result<ProduceResponseData> {
    let nt = (u8::arbitrary(u)? as usize) % MAX_ITEMS;
    ...
    F: FnOnce(&mut BytesMut) -> kafkaesque::error::Result<()>,
{
    ...
    match f(&mut buf) {
        ...
        Err(other) => panic!("encoder produced a non-Config error on adversarial input: {other:?}"),
    }
}
