...
fuzz_target!(|data: &[u8]| {
    ...
    if input.client_first.len() > MAX_PER_MESSAGE
        || input.client_final.len() > MAX_PER_MESSAGE
    {
        return;
    }
    ...
    // connection).
    let (final_state, _server_final_bytes) =
        handle_client_final(input.client_final, state);
