...
impl FailureDetector {
    ...
    pub fn check_brokers(&self) -> Vec<HealthStateChange> {
        ...
        // runtime starvation). The threshold is an *absolute* 2 s rather
        // than scaled by heartbeat_interval — at a 5 s interval, the
        // previous `10 × heartbeat_interval` rule needed a 50 s pause to
        // trip, so a 2 s GC pause silently translated into "every broker
        // missed heartbeats" and a cluster-wide spurious failover.
        // The 2 s constant comfortably exceeds normal tick spacing
        // (default check_interval is 250 ms) while catching the kind of
        // pause that actually distorts liveness observation.
        {
            let mut last = self.last_tick.lock().unwrap_or_else(|e| e.into_inner());
            if let Some(prev) = *last {
                let gap = now.saturating_duration_since(prev);
                const PAUSE_THRESHOLD: Duration = Duration::from_millis(2000);
                let pause_threshold = PAUSE_THRESHOLD;
                if gap > pause_threshold {
                    ...
                    // The pause is in the *monitor*, not the brokers — we
                    // have no idea whether anyone was actually unhealthy
                    // across the gap. Bumping last_heartbeat alone leaves a
                    // pre-pause Suspected state pinned forever (or until a
                    // heartbeat arrives), which churns metrics and can drive
                    // spurious failovers. Reset missed_count and clear
                    // suspicion; if the broker really is unhealthy the next
                    // few ticks will re-detect it.
                    for mut entry in self.brokers.iter_mut() {
                        let health = entry.value_mut();
                        health.last_heartbeat += gap;
                        health.missed_count = 0;
                        if health.state == BrokerHealthState::Suspected {
                            health.state = BrokerHealthState::Healthy;
                        }
                    }
                }
