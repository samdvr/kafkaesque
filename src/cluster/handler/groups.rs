...
pub(super) async fn handle_find_coordinator(
    ...
    // Place the group on a deterministic broker via consistent hashing
    // over the live broker set. Stable placement is what makes the
    // coordinator unique: all clients of the same group converge on the
    // same broker, so a single owner drives the group's transitions.
    // Pointing every client at "self" instead would let two brokers run
    // the rebalance protocol for the same group at the same time.
    let brokers = match handler.coordinator.get_live_brokers().await {
        ...
        Ok(_) | Err(_) => {
            return FindCoordinatorResponseData {
                ...
                error_message: Some("No live brokers available".to_string()),
                node_id: -1,
                host: String::new(),
                port: 0,
            };
    ...
    let chosen_id = match select_coordinator_id(&request.key, &brokers) {
        Some(id) => id,
        None => {
            return FindCoordinatorResponseData {
                throttle_time_ms: 0,
                error_code: KafkaCode::GroupCoordinatorNotAvailable,
                error_message: Some("Could not select a coordinator".to_string()),
                node_id: -1,
                host: String::new(),
                port: 0,
            };
    ...
    let chosen = brokers.iter().find(|b| b.broker_id == chosen_id);
    match chosen {
        Some(b) => FindCoordinatorResponseData {
            throttle_time_ms: 0,
        ...
        None => FindCoordinatorResponseData {
            throttle_time_ms: 0,
            error_code: KafkaCode::GroupCoordinatorNotAvailable,
            error_message: Some("Selected coordinator not in live broker set".to_string()),
            node_id: -1,
            host: String::new(),
            port: 0,
        },
...
pub(super) async fn handle_join_group(
    ...
    // Generate the member id only after all sync validation has passed.
