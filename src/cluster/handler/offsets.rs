...
pub(super) async fn handle_offset_commit(
    ...
    for topic in request.topics {
        ...
        for partition in &topic.partitions {
            ...
            let store_for_partition = handler
                .partition_manager
                .get_for_read(&topic.name, partition.partition_index)
                .await
                .ok();
            let store_hwm = store_for_partition.as_ref().map(|s| s.high_watermark());
            let bounds_error = match store_for_partition.as_ref() {
                Some(store) => {
