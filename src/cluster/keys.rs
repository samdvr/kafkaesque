...
pub use crate::protocol::{parse_record_count_checked, patch_base_offset};
...
///
/// `last_used_at_ms` records the wall-clock time of the most recent
/// successful append; the periodic producer-state retention sweep deletes
/// keys whose age exceeds the configured retention window. Legacy values
/// decode with the corresponding fields set to the "unknown" sentinel (-1).
...
pub struct PersistedProducerState {
    ...
    /// Wall-clock time (epoch millis) of the most recent successful append.
    /// -1 when unknown (legacy persisted values pre-dating the retention
    /// sweep). The retention sweep treats unknown timestamps as "use the
    /// partition open time" so legacy entries become eligible for deletion
    /// after a full retention cycle from broker upgrade.
    pub last_used_at_ms: i64,
}

/// Encode producer state value.
///
/// Format (v3): `[last_sequence:4][producer_epoch:2][last_first_sequence:4][last_base_offset:8][last_used_at_ms:8]`
/// (26 bytes total). Decoders accept the legacy 18-byte (v2) and 6-byte (v1)
/// prefix-only forms.
pub fn encode_producer_state_value(state: &PersistedProducerState) -> [u8; 26] {
    ...
    value[18..26].copy_from_slice(&state.last_used_at_ms.to_be_bytes());
    value
}

/// Decode producer state from a value.
///
/// Accepts the current 26-byte format and both legacy formats: 18-byte
/// (no `last_used_at_ms`) and 6-byte (no retry-dedup, no `last_used_at_ms`).
/// Missing fields decode as -1 / unknown.
pub fn decode_producer_state_value(value: &[u8]) -> Option<PersistedProducerState> {
    ...
    let last_used_at_ms = if value.len() >= 26 {
        i64::from_be_bytes(value[18..26].try_into().ok()?)
    } else {
        -1
    };
...
mod tests {
    ...
    fn test_producer_state_value_edge_cases() {
        ...
        let state = pstate_full(i32::MAX, i16::MAX, i32::MAX, i64::MAX, i64::MAX);
        assert_eq!(
        ...
        let state = pstate_full(-1, -1, -1, -1, -1);
        assert_eq!(
            decode_producer_state_value(&encode_producer_state_value(&state)),
            Some(state)
        );
        ...
        // Test with zero
        let state = pstate_full(0, 0, 0, 0, 0);
        assert_eq!(
            decode_producer_state_value(&encode_producer_state_value(&state)),
            Some(state)
        );
    ...
    fn test_producer_state_value_legacy_6_byte_decode() {
        ...
        // Retry-dedup fields and last_used_at_ms must decode as -1 (unknown).
        ...
        assert_eq!(
            ...
            Some(pstate_full(42, 3, -1, -1, -1))
        );
    }

    #[test]
    fn test_producer_state_value_legacy_18_byte_decode() {
        // v2 format: 18 bytes, no last_used_at_ms.
        // last_used_at_ms must decode as -1 (unknown).
        let mut v2 = Vec::new();
        v2.extend_from_slice(&42i32.to_be_bytes());
        v2.extend_from_slice(&3i16.to_be_bytes());
        v2.extend_from_slice(&40i32.to_be_bytes());
        v2.extend_from_slice(&1234i64.to_be_bytes());
        assert_eq!(
            decode_producer_state_value(&v2),
            Some(pstate_full(42, 3, 40, 1234, -1))
        );
    }
    ...
    proptest! {
        ...
        //
        // `decode_producer_state_value` accepts the current 26-byte (v3)
        // layout, the 18-byte (v2) layout, AND the legacy 6-byte (v1)
        // prefix-only layout. A bad decode here is silent data loss for
        // idempotent producers, so the boundary cases need explicit
        // coverage that point examples can't supply.
        // ----------------------------------------------------------------
        ...
        fn producer_state_decode_arbitrary_short_never_panics(
            ...
            // Catch-all robustness: any input up to ~1.5× the v3 size must
        ...
        fn producer_state_decode_legacy_6_byte(
            ...
            // retry-dedup fields and last_used_at_ms at the "unknown"
            // sentinel (-1).
            ...
            prop_assert_eq!(decoded.last_used_at_ms, -1);
        }

        #[test]
        fn producer_state_decode_v3_prefix_matches_legacy(
            seq in any::<i32>(),
            epoch in any::<i16>(),
            first_seq in any::<i32>(),
            base_offset in any::<i64>(),
            ...
            // For length ≥26, the (last_sequence, producer_epoch) prefix
            ...
            let v3 = encode_producer_state_value(&state);
            let v3_decoded = decode_producer_state_value(&v3)
                .expect("v3 value decodes");
            let v2_decoded = decode_producer_state_value(&v3[..18])
                .expect("v2 prefix decodes");
            let legacy_decoded = decode_producer_state_value(&v3[..6])
                .expect("legacy prefix decodes");
            prop_assert_eq!(v3_decoded.last_sequence, legacy_decoded.last_sequence);
            prop_assert_eq!(v3_decoded.producer_epoch, legacy_decoded.producer_epoch);
            prop_assert_eq!(v2_decoded.last_first_sequence, first_seq);
            prop_assert_eq!(v2_decoded.last_base_offset, base_offset);
            prop_assert_eq!(v2_decoded.last_used_at_ms, -1);
            prop_assert_eq!(legacy_decoded.last_first_sequence, -1);
            prop_assert_eq!(legacy_decoded.last_base_offset, -1);
            prop_assert_eq!(legacy_decoded.last_used_at_ms, -1);
        }

        #[test]
        fn producer_state_decode_partial_v3_returns_lower_format_fields(
            seq in any::<i32>(),
            epoch in any::<i16>(),
            extra in proptest::collection::vec(any::<u8>(), 0..20),
        ) {
            // Lengths 6..=25 are "between" formats. Lengths 6..18 honor
            // only the 6-byte prefix (retry + timestamp at -1). Lengths
            // 18..26 honor the v2 prefix (timestamp at -1).
            let mut v = Vec::with_capacity(6 + extra.len());
            v.extend_from_slice(&seq.to_be_bytes());
            v.extend_from_slice(&epoch.to_be_bytes());
            v.extend_from_slice(&extra);
            prop_assume!(v.len() < 26);
