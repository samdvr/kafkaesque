...
impl LoadMetricsCollector {
    ...
    pub fn record_produce(&self, topic: &str, partition: i32, bytes: u64, messages: u64) {
        // One key allocation per call: previously we paid two
        // `topic.to_string()` per call (once for the cap-check probe and once
        // for the entry). Build the key once and reuse for both.
        let key = (topic.to_string(), partition);
        if self.partitions.len() >= self.config.max_tracked_partitions
            && !self.partitions.contains_key(&key)
        {
            // Over the cap and we don't already track this partition: drop.
            return;
        }
        self.partitions
            .entry(key)
    ...
    pub fn record_fetch(&self, topic: &str, partition: i32, bytes: u64, messages: u64) {
        // Same single-allocation pattern as `record_produce`.
        let key = (topic.to_string(), partition);
        if self.partitions.len() >= self.config.max_tracked_partitions
            && !self.partitions.contains_key(&key)
        {
