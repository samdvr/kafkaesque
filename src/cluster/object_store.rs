...
use super::object_store_faults::{FaultInjector, FaultingObjectStore};
use super::object_store_metrics::MetricsObjectStore;
...
pub fn create_object_store(config: &ClusterConfig) -> SlateDBResult<Arc<dyn ObjectStore>> {
    create_object_store_with_faults(config, None)
}

/// Create an object store with an optional fault injector layered between
/// metrics recording and the cluster prefix. Production callers always
/// pass `None`; chaos tests pass `Some(injector)` and steer faults at
/// runtime by holding a clone of the injector handle.
pub fn create_object_store_with_faults(
...
fn layered_store(
    ...
    let metrics: Arc<dyn ObjectStore> = Arc::new(MetricsObjectStore::new(raw));
    match fault_injector {
        Some(injector) => Arc::new(FaultingObjectStore::new(metrics, injector)),
        None => metrics,
    }
}
