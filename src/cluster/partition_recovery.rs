...
use super::keys::{
    ...
    decode_producer_state_value, decode_record_offset, encode_record_key,
...
pub async fn recover_hwm_from_records(
    ...
    while let Some(item) = iter.next().await? {
        ...
        if let Some(offset) = decode_record_offset(&item.key) {
            ...
            let record_count = parse_record_count_checked(batch_data).unwrap_or(0);
            if record_count > 0 {
                ...
                // `checked_add` saturates to i64::MAX on overflow rather than
                // wrapping negative, which would silently underreport HWM and
                // allow a partition near i64::MAX to accept duplicate offsets
                // after recovery.
