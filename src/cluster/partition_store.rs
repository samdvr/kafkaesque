...
impl<'a> OffsetReservation<'a> {
    fn new(next_offset: &'a AtomicI64, base_offset: i64) -> Self {
        Self {
            next_offset,
            base_offset,
            armed: true,
...
impl BatchIndex {
    ...
    fn floor(&self, target: i64) -> Option<(i64, i32)> {
        let g = self.inner.read().unwrap();
        g.range(..=target).next_back().map(|(&k, &v)| (k, v))
    }

    fn insert(&self, base_offset: i64, record_count: i32) {
...
impl PartitionStore {
    ...
    async fn append_batch_inner(&self, records: &Bytes, durable: bool) -> SlateDBResult<i64> {
        ...
        if durable_write {
            super::metrics::record_slatedb_flush(
                &self.topic,
