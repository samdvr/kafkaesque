...
mod tests {
    ...
    async fn test_reject_zero_record_count_batch() {
        ...
        match result {
            Err(super::super::error::SlateDBError::CorruptBatch { reason, .. }) => {
                assert!(reason.contains("Invalid record count"));
            }
            _ => panic!("Expected CorruptBatch error for invalid record count"),
        }
    ...
    #[tokio::test]
    async fn test_prune_producer_states_deletes_stale_keys() {
        use super::super::keys::{
            PRODUCER_STATE_KEY_PREFIX, decode_producer_state_value, encode_producer_state_key,
        };

        let object_store: Arc<dyn object_store::ObjectStore> = Arc::new(InMemory::new());
        let store = PartitionStore::open(object_store, "test", "prune-topic", 0)
            .await
            .expect("Failed to open partition store");

        let producer_id = 42i64;
        let batch1 = create_test_batch(producer_id, 0, 0, 5);
        store
            .append_batch(&batch1)
            .await
            .expect("First batch failed");
        ...
        // Pruning with a long retention should leave the just-written entry
        // alone.
        let pruned = store
            .prune_producer_states(60_000, now_ms)
            .await
            .expect("Prune failed");
        assert_eq!(pruned, 0, "fresh entry must not be pruned");

        // Verify it survives a future-dated check that respects the cache.
        // (The in-memory cache holds the active producer; the sweep skips
        // any key still in cache regardless of on-disk timestamp.)
        let pruned = store
            .prune_producer_states(1, now_ms.saturating_add(86_400_000))
            .await
            .expect("Prune failed");
        assert_eq!(pruned, 0, "live producer in cache must not be pruned");

        // Drop the cache entry to simulate an idle producer; on-disk
        // timestamp is now the only signal.
        store.invalidate_producer_state_cache(producer_id);

        // Sweep with future-dated `now_ms` must delete the on-disk entry
        // because the persisted last_used_at_ms is far in the past.
        let far_future = now_ms.saturating_add(86_400_000);
        let pruned = store
            .prune_producer_states(60_000, far_future)
            .await
            .expect("Prune failed");
        assert_eq!(pruned, 1, "stale entry must be pruned");

        // Verify the on-disk key is actually gone.
        let key = encode_producer_state_key(producer_id);
        let bytes = store.raw_db_get(key.as_slice()).await;
        assert!(
            bytes.is_none(),
            "producer-state key must be deleted from SlateDB"
        );

        // Sentinel checks: prefix range is empty (no leftovers).
        let start = [PRODUCER_STATE_KEY_PREFIX];
        let end = [PRODUCER_STATE_KEY_PREFIX + 1];
        let count = store.raw_db_count_in_range(&start, &end).await;
        assert_eq!(count, 0, "no producer-state keys should remain");

        // Round-trip the encoded value to confirm the format we wrote
        // includes a non-negative last_used_at_ms (defense-in-depth: a
        // wrong-format value would be skipped by the sweep and the test
        // above would falsely pass).
        let batch_again = create_test_batch(producer_id, 0, 5, 5);
        store
            .append_batch(&batch_again)
            .await
            .expect("Second batch failed");
        let key = encode_producer_state_key(producer_id);
        let bytes = store
            .raw_db_get(key.as_slice())
            .await
            .expect("entry should exist after re-append");
        let persisted = decode_producer_state_value(&bytes).expect("encoded value decodes");
        assert!(
            persisted.last_used_at_ms >= 0,
            "last_used_at_ms must be stamped on append"
        );
    ...
    #[tokio::test]
    async fn test_prune_producer_states_disabled_when_retention_zero() {
        let object_store: Arc<dyn object_store::ObjectStore> = Arc::new(InMemory::new());
        let store = PartitionStore::open(object_store, "test", "prune-disabled", 0)
            .await
            .expect("Failed to open partition store");

        let batch = create_test_batch(1, 0, 0, 5);
        store.append_batch(&batch).await.expect("Batch failed");

        let pruned = store
            .prune_producer_states(0, i64::MAX)
            .await
            .expect("Prune with retention=0 must succeed and be a no-op");
        assert_eq!(pruned, 0);
        let pruned = store
            .prune_producer_states(-1, i64::MAX)
            .await
            .expect("Prune with negative retention must succeed and be a no-op");
        assert_eq!(pruned, 0);
