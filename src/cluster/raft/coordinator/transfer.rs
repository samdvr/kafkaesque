...
impl PartitionTransferCoordinator for RaftCoordinator {
    ...
    async fn batch_transfer_partitions(
        ...
        // Generate a unique transaction ID. Layout: broker_id (top 16 bits)
        // | per-broker monotonic counter (bottom 48 bits). The counter is
        // a full u64 internally; truncating to 48 bits gives ~281 trillion
        // transactions per broker before wrap, which at 1M ops/sec is over
        // 9 years. The timestamp travels in `timestamp_ms` on the command
        // itself; folding it into the id buys nothing and previously caused
        // a ~4.7-hour collision window.
        let counter = self
            .transaction_counter
