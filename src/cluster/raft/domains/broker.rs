        ...
        match cmd {
            ...
            BrokerCommand::Heartbeat {
                ...
                if let Some(broker) = self.brokers.get_mut(&broker_id) {
                    ...
                    // Only the deterministic state update lives here — apply
                    // runs on every replica and on crash-recovery replay, so
                    // metric/log emission must happen exactly once per real
                    // heartbeat at the call site, not inside the FSM.
