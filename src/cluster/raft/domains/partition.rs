...
///
/// Sibling domains (e.g. transfer) must share this clock so a transferred
/// lease's `lease_expires_at_ms` is comparable to subsequent reads gated on
/// `lease_clock_ms`.
#[inline]
pub(super) fn advance_lease_clock(lease_clock_ms: &mut u64, timestamp_ms: u64) -> u64 {
    *lease_clock_ms = (*lease_clock_ms).max(timestamp_ms);
    *lease_clock_ms
}

/// Bump `leader_epoch` with overflow protection.
///
/// Saturating at `i32::MAX` rather than wrapping because a wrapped negative
/// epoch silently disables write fencing: the per-write gate compares
/// `expected_epoch != 0` and a negative value would still fail that check on
/// a defense-in-depth gate, but the protocol-level comparisons that select
/// the higher of two epochs (`stored > expected`) become inverted on
/// negatives. Saturating preserves a deterministic ordering at the cost of
/// epoch resolution at the very high end (after ~2 billion takeovers a
/// partition stops gaining new fencing distinctions, which is acceptable —
/// the partition has bigger problems if it has reached that count).
#[inline]
pub(super) fn bump_leader_epoch(epoch: &mut i32) {
    let next = epoch.saturating_add(1);
    if next == *epoch {
        // Saturated. Log once per saturation event (not once per call) is
        // fine here — the FSM is replicated, every replica logs the same
        // line, and operators correlating across nodes get a clear signal.
        tracing::error!(
            current_epoch = *epoch,
            "leader_epoch saturated at i32::MAX; further takeovers cannot bump the epoch"
        );
    }
    *epoch = next;
}
...
/// Per-partition outcome inside a batch `RenewLeases` response.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum BatchRenewOutcome {
    /// Lease renewed; the new wall-clock expiry, in ms since UNIX epoch.
    Renewed { lease_expires_at_ms: u64 },
    /// Renewal rejected because the broker is no longer the owner. The
    /// caller should treat this as a "lease lost" signal — same as a
    /// single-partition `PartitionNotOwned`.
    NotOwned,
}
...
pub enum PartitionCommand {
    ...
    /// Raft proposal. The lease-renewal loop on a broker that owns N
    /// partitions used to issue N separate proposals (N FSM applies, N
    /// fsyncs); this carries the whole batch in one entry so the leader
    /// pays one commit per renewal cycle. Per-partition outcomes are
    /// reported via `LeasesRenewed::results`.
    RenewLeases {
        broker_id: i32,
...
pub enum PartitionResponse {
    ...
    /// `partitions[i]` in the request order. A partition that was renewed
    /// is reported as `Ok(lease_expires_at_ms)`; a partition that the
    /// broker no longer owns is reported as `Err(())` so the caller can
    /// treat it as a `LeaseLost` signal.
...
impl PartitionDomainState {
    ...
    pub fn apply(&mut self, cmd: PartitionCommand, lease_clock_ms: &mut u64) -> PartitionResponse {
        match cmd {
            ...
            PartitionCommand::RenewLeases {
                broker_id,
                partitions,
                lease_duration_ms,
                timestamp_ms,
            } => {
                // Advance the replicated lease clock once for the whole
                // batch: the renewal window applies to every partition in
                // this proposal.
                let now = advance_lease_clock(lease_clock_ms, timestamp_ms);
                    ...
                    let outcome = if let Some(state) = self.partitions.get_mut(&key) {
                        if state.owner_broker_id != Some(broker_id) {
                            ...
                            >= state
                                ...
                                .saturating_add(LEASE_TAKEOVER_GRACE_MS)
                        {
                            // Lease is past the grace window; this is the
                            // same skew-tolerance check as single-partition
                            // RenewLease — the owner must re-acquire.
                            BatchRenewOutcome::NotOwned
                        } else {
                ...
                PartitionResponse::LeasesRenewed { results }
            }
            ...
            PartitionCommand::ReleasePartition {
                    ...
                    && partition_state.owner_broker_id == Some(broker_id)
                {
                    ...
                    // Bump leader_epoch as the actual write fence: any in-flight
                    // write from the releasing broker against the prior epoch
                    // is rejected on the next append. Without this, a re-acquire
                    // by the same broker reuses the prior epoch and late writes
                    // can land against the new lease (split-brain window).
                    bump_leader_epoch(&mut partition_state.leader_epoch);
                }
            ...
            PartitionCommand::ExpireLeases { current_time_ms } => {
                ...
                for partition_state in self.partitions.values_mut() {
                        ...
                        && partition_state.lease_expires_at_ms <= now
                    {
                        partition_state.owner_broker_id = None;
                        partition_state.lease_expires_at_ms = 0;
                        // Same fence rationale as ReleasePartition: bump the
                        // epoch so writes still in flight from the formerly-
                        // owning broker cannot land against the next lease.
                        bump_leader_epoch(&mut partition_state.leader_epoch);
                        count += 1;
                    }
                }
    ...
    ///
    /// Bumps `leader_epoch` on every released partition as the write fence:
    /// any in-flight produce from the releasing broker against the prior
    /// epoch is rejected on the next append, preventing the late-write /
    /// split-brain window if the same broker re-acquires the partition.
