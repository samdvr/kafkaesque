...
use super::partition::bump_leader_epoch;
...
impl TransferDomainState {
    ...
    /// * `lease_clock_ms` - Replicated, monotonic lease clock shared with the
    ///   partition domain. New `lease_expires_at_ms` values are derived from
    ///   `max(*lease_clock_ms, timestamp_ms)` to keep them comparable to read
    ///   paths gated on the same clock; bypassing this would let a transferred
    ///   lease appear already-expired everywhere.
        ...
        match cmd {
            TransferCommand::TransferPartition {
                ...
                if let Some(partition_state) = partitions.get_mut(&key) {
                    ...
                    // Derive the lease deadline from the replicated, monotonic
                    // lease clock; using the raw `timestamp_ms` here lets a
                    // backward-skewed proposer set a deadline that's already
                    // in the past relative to read-path comparisons.
                    let now = super::partition::advance_lease_clock(lease_clock_ms, timestamp_ms);
                    partition_state.lease_expires_at_ms = now + lease_duration_ms;
            ...
            TransferCommand::BatchTransferPartitions {
                ...
                // Advance the replicated lease clock once per batch using
                // this command's `timestamp_ms`; subsequent per-transfer
                // leases derive their `lease_expires_at_ms` from the clamped
                // value. Bypassing the lease clock here renders the new
                // owner invisible to read paths gated on `lease_clock_ms`.
                let lease_now = super::partition::advance_lease_clock(lease_clock_ms, timestamp_ms);
                ...
                for transfer in transfers {
                    ...
                    if let Some(partition_state) = partitions.get_mut(&key) {
                            ...
                            || partition_state.owner_broker_id.is_none()
                        {
                            ...
                            bump_leader_epoch(&mut partition_state.leader_epoch);
                            partition_state.lease_expires_at_ms = lease_now + lease_duration_ms;
