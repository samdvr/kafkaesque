    ...
    pub async fn apply_command(&self, command: CoordinationCommand) -> CoordinationResponse {
        ...
        match command {
            ...
            CoordinationCommand::PartitionDomain(cmd) => {
                ...
                // Fire hooks AFTER releasing the inner state.write() guard.
                // The hook spawns onto the runtime and bumps cache
                // generations; running it under the global FSM write lock
                // would block every concurrent reader (`owns_partition_for_read`
                // takes state.read()) for the duration of the spawn — and a
                // panic inside the hook would unwind out of `apply_command`
                // with `state.version += 1` already executed but the response
                // not yet returned.
                drop(state);
            ...
            CoordinationCommand::TransferDomain(cmd) => {
                ...
                // same applied command. The four borrows are of disjoint
                // fields, so no cloning is needed.
                let state_mut = &mut *state;
                let response = state_mut.transfer_domain.apply_with_context(
                    cmd.clone(),
                    &mut state_mut.broker_domain.brokers,
                    &mut state_mut.partition_domain.partitions,
                    &mut state_mut.lease_clock_ms,
                );
                let invalidation = ownership_invalidation_for_transfer(&response);
                // See partition-domain branch for why hooks must fire AFTER
                // releasing the inner state.write() guard.
                drop(state);
                if let Some(invalidation) = invalidation {
                    self.fire_ownership_change_hook(invalidation);
                }
