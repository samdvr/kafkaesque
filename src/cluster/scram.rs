...
/// Per-process random key used to derive deterministic-but-unforgeable
/// dummy salts for unknown-user SCRAM handshakes. The salt for a missing
/// user is `HMAC(dummy_salt_key, username)[..SALT_LEN]`, which:
///
/// * is stable for the lifetime of the process (a probe that retries with
///   the same username sees the same salt — same as a real account),
/// * cannot be predicted by an off-host attacker (the key is OS-random),
/// * cannot be replayed across restarts (a new key is minted every startup,
///   matching the pattern a fresh real account would exhibit).
fn dummy_salt_key() -> &'static [u8] {
...
impl ScramCredentials {
    ...
    /// Iterations below [`MIN_ITERATIONS`] are rounded up — accepting them
    /// would silently produce credentials weaker than the SCRAM-SHA-256
    /// floor, regardless of how this entry-point is reached (env override,
    /// snapshot replay, programmatic construction).
...
pub fn handle_client_first(
    ...
    let Some(creds) = creds_lookup(&username) else {
        // Don't reveal user existence: produce a server-first as if the user
        // existed. The salt and iteration count must be indistinguishable
        // from a real account or the response itself is the oracle —
        //   * salt: keyed HMAC over the username, so a probe that retries
        //     with the same name sees the stable per-user salt a real
        //     account would emit, while an off-host attacker can neither
        //     predict the value nor reuse it across processes;
        //   * iterations: whatever a real `add_user` would have used
        //     (`configured_iterations()`), so the `i=` field doesn't fork
        //     the shape of the response.
        let mut salt = vec![0u8; SALT_LEN];
        salt.copy_from_slice(&hmac_sha256(dummy_salt_key(), username.as_bytes())[..SALT_LEN]);
