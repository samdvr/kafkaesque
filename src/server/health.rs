...
pub struct HealthServer {
    ...
    /// Optional direct query handle to the Raft layer. When present, the
    /// readiness probe consults this instead of the global `RAFT_STATE`
    /// gauge, which lags by up to one polling interval (1s) and depends on
    /// the coordinator remembering to publish on startup. Falling back to
    /// the gauge keeps the bare `HealthServer::new(...)` code path working
    /// for users who don't want to wire the coordinator in.
    raft_state_provider: Option<Arc<dyn RaftStateProvider>>,
...
/// Direct query handle the health server uses to ask the Raft layer for its
/// current state, bypassing the global `RAFT_STATE` gauge. The gauge has a
/// ~1-second update lag (set by a periodic poller on `RaftCoordinator`),
/// and depends on the poller actually running and publishing — not great
/// when the readiness probe is the thing deciding whether traffic flows.
///
/// Implementors should map their internal state to the same integer codes
/// the gauge uses: `0` (Follower) and `2` (Leader) → ready; everything else
/// → not ready.
pub trait RaftStateProvider: Send + Sync + 'static {
    /// Return the broker's current Raft state code. See
    /// [`crate::cluster::metrics::is_raft_ready`] for the encoding.
...
impl HealthServer {
    ...
    /// Provide a direct query handle to the Raft layer. When set, the
    /// readiness probe consults this instead of the lagged-by-1-second
    /// global `RAFT_STATE` gauge. Optional — leaving it unset preserves
    /// the gauge-fallback behavior.
    pub fn with_raft_state_provider(mut self, provider: Arc<dyn RaftStateProvider>) -> Self {
        self.raft_state_provider = Some(provider);
        self
    }
    ...
    /// mode that prior versions of `is_ready` *missed*
    ///
                ...
                accept_result = self.listener.accept() => {
                    match accept_result {
                        Ok((mut stream, addr)) => {
                            ...
                            let raft_provider = self.raft_state_provider.clone();
...
fn handle_request(
    ...
    raft_state_provider: Option<&dyn RaftStateProvider>,
) -> String {
    ...
    match path {
        ...
        "/ready" | "/readyz" | "/ready/" => {
            ready_response(zombie_state, broker_id, raft_state_provider)
        }
        "/metrics" => {
...
fn ready_response(
    ...
    // Raft consensus check: a broker that's a Candidate (no leader yet) or
    // Shutdown can't act on cluster operations even if zombie mode is off
    // and the object store is up. Without this, a broker stuck mid-election
    // still reports ready=200 and Kubernetes routes traffic to it; clients
    // see BROKER_NOT_AVAILABLE storms instead of clean redirection.
    //
    // Prefer a direct provider query when wired (current Raft state with no
    // up-to-1s gauge lag). Fall back to the global gauge so the gauge-only
    // construction path still works.
    let raft_ready = match raft_state_provider {
        Some(p) => matches!(p.current_state(), 0 | 2),
        None => crate::cluster::metrics::is_raft_ready(),
    };
    if !raft_ready {
        return format!(
            "HTTP/1.1 503 Service Unavailable\r\n\
...
mod tests {
    ...
    /// RAII guard that publishes a Follower (ready) Raft state for the
    /// duration of a `/ready`-expecting test, then restores the global
    /// gauge to the uninitialized sentinel on drop. Tests that assert
    /// `/ready` returns 200 must hold one of these — the global metric is
    /// shared across the test process, so leaking the value would make
    /// later tests that assert `503 raft_not_in_quorum` non-deterministic.
    struct RaftReadyGuard;

    impl RaftReadyGuard {
        fn follower() -> Self {
            crate::cluster::metrics::set_raft_state(0);
            Self
        }
    }

    impl Drop for RaftReadyGuard {
        fn drop(&mut self) {
            crate::cluster::metrics::set_raft_state(
                crate::cluster::metrics::RAFT_STATE_UNINITIALIZED,
            );
        }
    }
    ...
    fn test_handle_ready_request_healthy() {
        let _raft = RaftReadyGuard::follower();
        let state = healthy();
    ...
    fn test_metrics_auth_correct_token() {
        ...
        assert!(response.contains("200 OK"));
        // Must not be the rejection path. Check the status line specifically —
        // the metrics body comes from the shared global registry and can
        // legitimately contain the substring "401" (a counter value, a label),
        // so a bare `!contains("401")` is order-dependent and flaky.
        assert!(!response.contains("401 Unauthorized"));
    }
    ...
    fn test_ready_response_healthy() {
        let _raft = RaftReadyGuard::follower();
        let state = healthy();
        let response = ready_response(&state, Some(5), None);
        assert!(response.contains("200 OK"));
        assert!(response.contains("status: ready"));
    ...
    #[test]
    fn test_ready_response_raft_candidate_returns_503() {
        // A broker mid-election has no leader to coordinate with; we must
        // not draw traffic to it. Set RAFT_STATE = Candidate (1) and verify
        // /ready reports raft_not_in_quorum, not 200.
        crate::cluster::metrics::set_raft_state(1);
        let state = healthy();
        let response = ready_response(&state, Some(5), None);
        // Restore the sentinel before any assertion can short-circuit, so a
        // failure here doesn't poison later tests' shared global state.
        crate::cluster::metrics::set_raft_state(crate::cluster::metrics::RAFT_STATE_UNINITIALIZED);
        assert!(response.contains("503 Service Unavailable"));
        assert!(response.contains("status: not_ready"));
    ...
    #[test]
    fn test_ready_response_raft_uninitialized_returns_503() {
        // A broker that hasn't yet polled Raft state should fail readiness
        // until the coordinator's first metrics tick lands.
        crate::cluster::metrics::set_raft_state(crate::cluster::metrics::RAFT_STATE_UNINITIALIZED);
        let state = healthy();
        let response = ready_response(&state, Some(5), None);
        assert!(response.contains("503 Service Unavailable"));
    ...
    fn test_handle_empty_request() {
        let state = healthy();
        let response = handle_request("", &state, None, None, None);
        assert!(response.contains("404 Not Found"));
    }
    ...
    fn test_handle_malformed_request() {
        let state = healthy();
        let response = handle_request("INVALID", &state, None, None, None);
        assert!(response.contains("404 Not Found"));
    }
