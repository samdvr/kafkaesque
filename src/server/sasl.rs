...
impl SaslAuthenticator for PlainAuthenticator {
    ...
    async fn authenticate(
        ...
        // shape (compare a 16-byte attacker password against an empty
        // dummy) measurably differed from comparing against a real
        // 16-byte stored password — letting a network attacker
        // distinguish "unknown user" from "user exists, wrong password"
        // (and even enumerate stored password lengths) by request timing.
        //
        // We pad both sides to a fixed `PAD_LEN` larger than any plausible
        // password and compare exactly that many bytes. Then the present-
        // user-but-wrong-password and absent-user paths run the same
        // amount of work.
        ...
        let candidate_bytes = stored.map(|s| s.as_bytes()).unwrap_or(&[]);
        ...
        // Compare both the padded buffer and the original lengths so a
        // password whose first PAD_LEN bytes match a longer stored value
