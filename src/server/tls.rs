...
/// Warn if a TLS private key file is readable beyond owner. A key with
/// permissive bits is silently exfiltratable by any local user; the
/// warning is the loudest signal we can emit without breaking deployments
/// that intentionally distribute keys via group-readable secrets.
#[cfg(unix)]
fn warn_if_world_readable(path: &Path) {
        ...
        && meta.permissions().mode() & 0o077 != 0
    {
        tracing::warn!(
            ...
            "TLS private key is readable by group or other; tighten with `chmod 600`"
        );
    }
}
