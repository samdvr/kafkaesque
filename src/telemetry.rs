...
///
/// Prefer [`init`] when both fmt and OpenTelemetry layers should be live in
/// the same process — calling `init_logging` followed by `init_telemetry`
/// silently drops the OTel layer because `try_init` only succeeds once.
///
/// # Example
///
/// ```rust,no_run
...
/// Initialize logging and (optionally) OpenTelemetry as a single tracing
/// subscriber.
///
/// `tracing_subscriber`'s `try_init` succeeds once per process: calling
/// [`init_logging`] and then [`init_telemetry`] silently drops the OTel
/// layer because the second `try_init` returns `Err` and operators see
/// no spans in their collector. Use this entry point when both layers
/// should be active — it builds one `Registry` with the fmt layer and,
/// if `otel_config` is provided and the `otel` feature is compiled in,
/// the OpenTelemetry layer as well.
///
/// When the `otel` feature is disabled, `otel_config` is ignored.
pub fn init(
    format: LogFormat,
    #[cfg_attr(not(feature = "otel"), allow(unused_variables))] otel_config: Option<
        TelemetryConfig,
    >,
    ...
    #[cfg(feature = "otel")]
    let otel_tracer = match otel_config.as_ref() {
        Some(config) => Some(build_otel_tracer(config)?),
        None => None,
    };
    ...
    match format {
        LogFormat::Json => {
            ...
            #[cfg(feature = "otel")]
            {
                let otel_layer = otel_tracer.map(OpenTelemetryLayer::new);
...
///
/// Prefer [`init`] when this process also needs fmt-layer logging — the
/// two-step `init_logging` + `init_telemetry` flow drops the second
/// subscriber silently because `try_init` only succeeds once.
///
/// # Errors
///
...
pub fn init_telemetry(config: TelemetryConfig) -> Result<(), Box<dyn std::error::Error>> {
    init(LogFormat::default(), Some(config)).map_err(|e| -> Box<dyn std::error::Error> { e })
}
