//! Mock-only chaos smoke tests for Kafkaesque.
//!
//! **What this file actually exercises.** Every test in this module drives
//! a `MockCoordinator` and a private `FaultInjector` whose state is
//! consulted only by other code in this same file. Nothing here flows
//! through the real Raft replication path or the real object-store IO
//! path. A "network partition" test mutates a `HashMap<Instant>`; a
//! "node crash" test calls `mock_coordinator.expire_my_leases().await`.
//! These assertions are valuable as a high-level state-machine sanity
//! check, but they are **smoke tests, not chaos coverage**.
//!
//! **Where real chaos coverage lives.**
//!
//! - `tests/object_store_chaos_tests.rs` — drives injected
//!   [`kafkaesque::cluster::FaultInjector`] / [`FaultingObjectStore`]
//!   faults through a real `RaftCoordinator` over `InMemory`. Latency,
//!   block, and forced-failure faults *actually* reach production
//!   `put`/`get` paths there.
//! - `tests/raft_chaos_starter.rs` — real Raft + `InMemory` chaos
//!   scenarios; the seed for porting more of the mock-only scenarios
//!   below to a real-coordinator harness.
//! - `tests/produce_leader_failover_tests.rs` — durability invariant for
//!   produce/failover, including the in-flight produce-during-failover
//!   race.
//! - `tests/linearizability_real_tests.rs` — single-node and multi-node
//!   linearizability tests against the real `RaftCoordinator`.
//!
//! New chaos scenarios that need to assert anything beyond
//! coordinator-state invariants should be added to one of those files,
//! not here. Tests added here are appropriate when the goal is a fast
//! state-machine sanity check that doesn't require spinning up Raft.
//!
//! # Test Categories
//!
//! 1. **Network Partitions** — mock partition_owners map mutation
//! 2. **Node Failures** — mock lease expiry
//! 3. **Zombie Mode** — mock zombie-mode flag transitions
//! 4. **Lease and Ownership** — mock partition acquisition transitions
//! 5. **SlateDB Fencing** — uses real `PartitionStore` + `InMemory`
//! 6. **Clock Skew** — mock lease clock manipulation
//! 7. **Resource Exhaustion** — mock backpressure counters
//! 8. **Recovery Scenarios** — mock state transitions
//!
//! # Running
//!
//! ```bash
...
mod partition_rebalance {
    ...
    /// Verify that a partition transfer concurrent with a consumer-group
    /// rebalance leaves both subsystems internally consistent
    ///
    /// Why this test exists: consumer-group state (members, generation,
    /// assignments) and partition-ownership state are persisted through the
    /// same coordinator but updated by independent commands. A regression
    /// that lets one update see a stale view of the other can produce
    /// "assignment references partition X owned by broker A, but partition
    /// X is now owned by broker B" — symptoms include consumer fetches
    /// hitting NotLeaderForPartition immediately after a successful sync.
    /// This test races the two operations and asserts the post-condition
    /// invariants hold.
    #[tokio::test]
    async fn test_rebalance_consistent_with_concurrent_partition_transfer() {
        use kafkaesque::cluster::raft::TransferReason;
        use kafkaesque::cluster::{ConsumerGroupCoordinator, PartitionTransferCoordinator};

        let cluster = TestCluster::new(2).await;
        cluster.create_topic("churn-rebalance", 4).await;

        // Initial ownership: broker 1 owns 0,1; broker 2 owns 2,3.
        for p in 0..2 {
            assert!(
                cluster
                    .broker(1)
                    .acquire_partition("churn-rebalance", p, 60)
                    .await
                    .unwrap()
            );
        }
        ...
        // First consumer joins — establishes a baseline generation. Both
        // joins target broker(1)'s coordinator because TestCluster brokers
        // do not share consumer-group state (only partition ownership is
        // shared via the cloned Arc), so generation bumps must be observed
        // on a single coordinator instance.
        let (gen1, _member1, _, _, _) = cluster
            .broker(1)
            .join_group("churn-grp", "consumer-1", &[], 30000)
            .await
            .unwrap();

        // Race a second consumer join (drives a generation bump and a
        // rebalance) against a partition transfer. The two operations touch
        // different MockCoordinator data structures (consumer_groups vs
        // partition_owners) but share the broker registry; under regression
        // either side could observe a half-consistent intermediate state.
        let cluster_arc = std::sync::Arc::new(cluster);
        let cluster_for_join = cluster_arc.clone();
        let join_handle = tokio::spawn(async move {
            cluster_for_join
                .broker(1)
                .join_group("churn-grp", "consumer-2", &[], 30000)
                .await
                .unwrap()
        });

        let cluster_for_transfer = cluster_arc.clone();
        let transfer_handle = tokio::spawn(async move {
            cluster_for_transfer
                .broker(1)
                .transfer_partition(
                    "churn-rebalance",
                    0,
        ...
        let (gen2, _member2, _, _, _) = join_handle.await.unwrap();
        transfer_handle
            .await
            .unwrap()
            .expect("transfer should succeed");

        // Invariant 1: rebalance generation strictly advanced.
        assert!(
            gen2 > gen1,
            "Generation must advance after second consumer joins (got {} → {})",
            gen1,
            gen2
        );

        // Invariant 2: every partition has exactly one owner.
        ...
        for p in 0..4 {
            ...
            for b in 1..=2 {
                if cluster_arc
                    .broker(b)
                    .owns_partition_for_read("churn-rebalance", p)
                    .await
                    .unwrap()
                {
                    owners += 1;
            ...
            assert_eq!(
                owners, 1,
                "partition {} ended with {} owners (expected exactly 1)",
                p, owners
            );
            total_owners += owners;
        }
        assert_eq!(total_owners, 4);

        // Invariant 3: the transferred partition reflects the new owner.
        assert_eq!(
            cluster_arc.get_owner("churn-rebalance", 0).await,
            Some(2),
            "partition 0 should be owned by broker 2 after transfer"
        );

        // Invariant 4: the consumer-group records exactly two members.
        let members = cluster_arc
            .broker(1)
            .get_group_members("churn-grp")
            .await
            .unwrap();
        assert_eq!(
            members.len(),
            2,
            "Group should have 2 members after both joins"
        );
    }
}
