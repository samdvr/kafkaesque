...
const TEST_RAFT_JOIN_TOKEN: &str = "test-raft-join-token-32-bytes-minimum!!";
...
fn test_from_env_production_profile_with_secret_succeeds() {
    with_env_vars(
        &[
            ...
            ("RAFT_JOIN_TOKEN", TEST_RAFT_JOIN_TOKEN),
            ("ACL_ENABLED", "true"),
        ],
        || {
            let config = ClusterConfig::from_env().expect("Should parse config");
...
fn test_from_env_production_open_broker_rejected() {
    with_env_vars(
        ...
        || {
            ...
            let err = result.unwrap_err().to_string();
            assert!(
                err.contains("ACL_ENABLED"),
                "error should mention missing ACL enforcement, got: {err}"
            );
        },
    );
}
...
fn test_from_env_sasl_required_with_enabled_succeeds() {
    with_env_vars(
        &[
            ...
            ("SASL_ENABLED", "true"),
            ("SASL_REQUIRED", "true"),
            // Production gate now requires ACL_ENABLED — SASL alone is
...
#[test]
fn test_from_env_production_profile_requires_join_token() {
    with_env_vars(
        &[
            ("BROKER_ID", "1"),
            ("HOST", "localhost"),
        ...
        || {
            ...
            assert!(
                result.is_err(),
                "production profile without join token must fail"
            );
            let err = result.unwrap_err().to_string();
            assert!(
                err.contains("RAFT_JOIN_TOKEN"),
                "error should name the missing join token, got: {err}"
            );
        },
    );
}
...
#[test]
fn test_from_env_production_rejects_multi_broker_local_storage() {
    with_env_vars(
        &[
            ("BROKER_ID", "0"),
            ("HOST", "localhost"),
            ...
            ("RAFT_PEERS", "0=broker0:9093,1=broker1:9093,2=broker2:9093"),
        ],
        || {
            let result = ClusterConfig::from_env();
            assert!(
                result.is_err(),
                "multi-broker local storage must fail in production profile"
            );
            let err = result.unwrap_err().to_string();
            assert!(
                err.contains("OBJECT_STORE_TYPE=local"),
                "error should mention local storage, got: {err}"
            );
        },
    );
}
