...
#[test]
fn test_count_configured_raft_peers() {
    assert_eq!(ClusterConfig::count_configured_raft_peers(None), 0);
    assert_eq!(
        ClusterConfig::count_configured_raft_peers(Some(&"0=h:1".to_string())),
        1
    );
    assert_eq!(
        ClusterConfig::count_configured_raft_peers(Some(&"0=h0:1,1=h1:1,2=h2:1".to_string())),
        3
    );
}
