...
// ============================================================================
// Oversized Frame / OOM Vector Tests
// ============================================================================

/// Verify that a client declaring a frame size larger than the configured
/// `max_message_size` is rejected at the framing layer *before* any buffer
/// proportional to the declared size is allocated.
///
/// Why this test exists: length-prefixed protocols are a classic
/// OOM vector. Without an explicit per-frame guard a malicious client can
/// send a 4-byte header declaring a 50 MB body and force the broker to
/// allocate 50 MB before the body even arrives. The frame reader
/// short-circuits on the size check, so this test pins the contract: the
/// reader rejects oversized frames at the size step, returning an error
...
async fn test_oversized_frame_rejected_before_allocation() {
    ...
    // 1 MB cap; client declares a 50 MB body.
    const MAX_MESSAGE_SIZE: usize = 1024 * 1024;
    const DECLARED_SIZE: i32 = 50 * 1024 * 1024;
    ...
    client
        ...
        .await
        .unwrap();
    // Intentionally do NOT write the body — if the reader were to allocate
    // and try to read 50 MB, the test would hang. Rejection must happen at
    // the size check.
    drop(client);
    ...
    let result = tokio::time::timeout(
        ...
        read_kafka_frame_for_fuzz(&mut server, MAX_MESSAGE_SIZE),
    )
    .await
    .expect("frame reader must short-circuit on oversized size, not block reading body");

    assert!(
        result.is_err(),
        "Expected oversized frame to be rejected, got Ok"
    );
    let err_str = result.unwrap_err().to_string();
    assert!(
        err_str.contains("exceeds maximum") || err_str.contains("exceeds"),
        "Error should mention exceeding max size, got: {}",
        err_str
    );
}

/// A frame that *fits* the configured cap is accepted. Sanity check that the
/// rejection in the previous test isn't a false-positive from an unrelated
/// framing bug.
#[tokio::test]
async fn test_frame_at_cap_accepted() {
    ...
    let (mut client, mut server) = tokio::io::duplex(MAX_MESSAGE_SIZE * 2);
    ...
    .await
    .expect("read should not hang for a within-budget frame")
    .expect("frame within max_message_size must be accepted");

    assert_eq!(result.len(), payload.len());
}

/// Negative frame sizes are rejected as protocol errors, never wrapped to
/// `usize` and used to drive an allocation.
#[tokio::test]
async fn test_negative_frame_size_rejected() {
    use kafkaesque::server::read_kafka_frame_for_fuzz;
    use tokio::io::AsyncWriteExt;
    ...
    client.write_all(&bad_size.to_be_bytes()).await.unwrap();
    drop(client);
    ...
    .await
    .expect("reader must not hang on a negative size");

    assert!(result.is_err(), "negative frame size must be rejected");
}
