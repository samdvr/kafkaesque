...
fn test_to_kafka_code_fenced_producer() {
    ...
    // Fenced producer maps to InvalidProducerEpoch (47); clients abort the
    // producer and re-init on this code rather than retrying the same epoch.
    assert_eq!(err.to_kafka_code(), KafkaCode::InvalidProducerEpoch);
}
...
fn test_to_kafka_code_sequence_overflow() {
    ...
    // Sequence overflow → UnknownProducerId (59); clients re-init.
    assert_eq!(err.to_kafka_code(), KafkaCode::UnknownProducerId);
}
