//! Smoke-level linearizability tests against an in-process `BTreeMap` mock.
//!
//! **For the real coverage, see `tests/linearizability_real_tests.rs`** —
//! that file drives the same scenarios against the real `PartitionStore`
//! and `RaftCoordinator` and verifies the histories with
//! `stateright::semantics::LinearizabilityTester` (Wing-Gong-Lynch — the
//! same algorithm Porcupine and Knossos implement).
//!
//! This file is retained because it's cheap (no SlateDB, no Raft) and
//! catches gross regressions in the harness shape itself. New scenarios
//! should land in `linearizability_real_tests.rs` instead flagged that any linearizability bug in the real partition
//! store / coordinator is invisible to a `BTreeMap+Mutex` shim.
//!
//! # Linearizability Properties Tested (against the mock)
//!
