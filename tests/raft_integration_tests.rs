...
// ============================================================================
// Snapshot install on learner join
// ============================================================================

/// Verify that a learner joining an established cluster receives and installs
/// the leader's snapshot, then catches up to the committed FSM state.
///
/// Why this test exists: the existing tests cover snapshot
/// *building* and snapshot *deserialization* in isolation, but never the
/// end-to-end "leader has compacted past the learner's log start, so openraft
/// must ship a snapshot for the learner to catch up" path. That path
/// exercises the most subtle correctness logic in the storage layer
/// (`install_snapshot` lock ordering, persist-before-swap, last_applied_log
/// advance) — without a test, regressions surface only in production.
#[tokio::test]
async fn test_learner_join_receives_and_installs_snapshot() {
    let leader_port = next_port();
    let learner_port = next_port();
    let mut leader_config = test_config(1, leader_port);
    let mut learner_config = test_config(2, learner_port);

    // Lower the threshold so the test triggers a snapshot quickly.
    leader_config.snapshot_threshold = 25;
    learner_config.snapshot_threshold = 25;

    // Both nodes must share an object store so the learner can fetch
    // snapshot data published by the leader. WAL/log dirs are per-node
    // (already separate via test_config's tempdir).
    let store = create_object_store();

    let leader = Arc::new(
        RaftNode::new(leader_config, store.clone(), Handle::current())
            .await
            .unwrap(),
    );
    leader.initialize_cluster().await.unwrap();
    assert!(
        wait_for_node_to_be_leader(&leader, Duration::from_secs(5)).await,
        "Leader should have been elected"
    );

    // Drive the leader well past snapshot_threshold so log compaction kicks
    // in. Producer-ID allocation is convenient because it's idempotent at
    // the allocation level (each call advances a counter) and confirms the
    // FSM is the same on both nodes.
    ...
    for _ in 0..120 {
        ...
        {
            allocated_ids.push(producer_id);
        }
    }
    assert_eq!(allocated_ids.len(), 120);

    // Give openraft a moment to schedule a snapshot build after crossing the
    // threshold. The actual snapshot is built asynchronously on a background
    // task in openraft.
    sleep(Duration::from_secs(1)).await;

    // Bring up the learner and register it with the leader.
    let learner = Arc::new(
        RaftNode::new(learner_config, store.clone(), Handle::current())
            .await
            .unwrap(),
    );

    leader
        .add_learner(2, format!("127.0.0.1:{}", learner_port))
        .await
        .expect("add_learner should succeed");

    // The learner is now part of the cluster; allocate one more ID. After
    // this completes, the commit index has advanced past the snapshot.
    let post_join_id = leader
        .write(CoordinationCommand::ProducerDomain(
            ProducerCommand::AllocateProducerId {
                request_token: None,
            },
        ...
        .await
        .unwrap();
    if let CoordinationResponse::ProducerDomainResponse(ProducerResponse::ProducerIdAllocated {
        producer_id,
        ..
    }) = post_join_id
    {
        ...
        panic!("expected ProducerIdAllocated, got {:?}", post_join_id);
    }

    // Promote learner to voter — change_membership requires the learner to
    // have caught up sufficiently, which transitively requires a successful
    // snapshot install when the learner started behind the snapshot. This
    // is the strongest single assertion that the install_snapshot path
    // worked end-to-end.
    let promote_deadline = Duration::from_secs(15);
    let start = std::time::Instant::now();
    let mut promoted = false;
    while start.elapsed() < promote_deadline {
        if leader.change_membership([1, 2]).await.is_ok() {
            promoted = true;
            break;
        }
        sleep(Duration::from_millis(200)).await;
    ...
    assert!(
        promoted,
        "Learner should catch up via snapshot + log within {:?}",
        promote_deadline
    );

    // Final sanity: a write through the leader still succeeds — the cluster
    // is healthy after the membership change.
    let final_response = leader
        .write(CoordinationCommand::ProducerDomain(
            ProducerCommand::AllocateProducerId {
                request_token: None,
            },
        ...
        .await
        .expect("write after promotion should succeed");
    assert!(matches!(
        final_response,
